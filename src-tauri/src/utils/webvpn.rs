...
pub fn translate_up(raw_url: &str) -> String {
    ...
    format!("{}/{}/{}{}", VPN_HOST, protocol, encrypted, path)
}

pub fn translate_down(vpn_url: &str) -> String {
    let parsed = Url::parse(vpn_url).unwrap();
    let mut seg = parsed.path_segments().unwrap();

    let protocol = seg.next().unwrap();
    let enc_host = seg.next().unwrap();

    let host = decrypt_host(enc_host);
...
mod tests {
    ...
    fn test_translate_up() {
        ...
        let translated = translate_up(url);
        assert_eq!(translated, "https://elib.ustb.edu.cn/http/77726476706e69737468656265737421a2a713d275603c1e2a50c7face/");
        let url = "https://space.bilibili.com/13161874?spm_id_from=333.1007.0.0";
        let translated = translate_up(url);
        assert_eq!(translated, "https://elib.ustb.edu.cn/https/77726476706e69737468656265737421e3e7409f227e6a5972018ba5945c6d36db05/13161874");
    }
    ...
    fn test_translate_down() {
        ...
        let url = translate_down(translated);
