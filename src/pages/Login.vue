  ...
  <div>
    <n-space vertical>
      <div v-if="!login_state">
        ...
        <n-grid :x-gap="5" :y-gap="8" :cols="6" style="margin-top: 10px">
          <n-grid-item :span="3">
            <n-input v-model:value="user_name" type="text" placeholder="学号/工号" round /></n-grid-item>
          <n-grid-item :span="3">
            <n-input v-model:value="password" type="password" show-password-on="mousedown" placeholder="密码"
              round /></n-grid-item>
          <n-grid-item :span="3">
            <n-button strong secondary type="primary" @click="get_cookies" :disabled="button_disabled">
              登陆校园网后台获取统计数据 ⭐️
            </n-button> </n-grid-item><n-grid-item>
            <n-dropdown trigger="hover" :options="options" @select="handleSelect">
              <n-button type="info">选择账号
                <n-icon size="20">
                  <ChevronDownOutline />
                </n-icon></n-button> </n-dropdown></n-grid-item><n-grid-item :span="2">
        ...
        <n-h3 prefix="bar" type="success" style="margin-top: 15px" v-if="button_disabled === true">登录中...</n-h3>
      </div>
      <div v-else>
        <n-h3 prefix="bar" type="success" style="margin-top: 15px">您已登录 {{ store.userName
      ...
      <n-grid :x-gap="12" :y-gap="8" :cols="2" style="margin-top: 10px">
        <n-grid-item>
          ...
          <n-card title="手动检查更新" hoverable @click="manually_check_update" class="my-card">
            手动地检查更新。
          </n-card> </n-grid-item><n-grid-item>
        ...
        <n-grid-item>
          <n-card title="一键切换登录校园网账号" hoverable @click="switchModal = true" class="my-card">
            会自动注销当前账号校园网登录，并尝试登录你选择的账号。
          </n-card>
        </n-grid-item>
        <n-grid-item>
          <n-card title="备份配置文件到 Onedrive" hoverable @click="open_microsoft_login" class="my-card">
        ...
        <n-grid-item>
          <n-card title="校园网使用指南" hoverable @click="open_guide" class="my-card">
            信息办的微信公众号出的。但是没有任何有用信息。
          </n-card>
        </n-grid-item>
      </n-grid>
    </n-space>
    <n-float-button :right="20" :bottom="20" type="primary" @click="showModal = true">
      <n-icon>
        <ColorPaletteOutline />
      </n-icon>
    </n-float-button>
    <n-modal v-model:show="showModal">
      <n-card style="margin: auto 50px">
        ...
        <n-button strong secondary type="primary" style="margin-left: 50px" @click="reset_background_image">
          去掉背景图片
        </n-button>
        <n-p>
          背景图片透明度: <br /><br /><n-slider :on-dragend="set_background_transparence" v-model:value="transparence"
            :default-value="0" :step="1" />
        </n-p>
        <n-p>
          背景图片模糊程度: <br /><br /><n-slider :on-dragend="set_background_blur" v-model:value="blur" :default-value="0"
            :step="1" />
        </n-p>
    ...
    <n-modal v-model:show="switchModal">
      <n-card style="margin: auto 50px">
        <n-p>请选择一个账号（如果这里没有，你需要先登录校园网后台获取该账号的统计数据，然后这个选项就有了）：</n-p>
        <n-dropdown trigger="hover" :options="options" @select="handleSelectSwitch">
          <n-button type="info">选择账号
            <n-icon size="20">
              <ChevronDownOutline />
            </n-icon></n-button>
        </n-dropdown>
        <div v-if="switchToUserName !== '' && store.userName !== ''" style="margin-top: 10px">
          <n-statistic label="你确定要切换账号么？此操作会从当前设备注销左侧账号，并尝试登录右侧的账号。">
            {{ store.userName }}
          ...
          <n-button style="margin-top: 10px" type="warning" @click="switchLoginUstbWifi">是</n-button>
        </div>
      </n-card>
    </n-modal>
  </div>
